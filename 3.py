...
def genetic_algorithm(N, population_size=100, generations=500, mutation_rate=0.05):
    ...
    for generation in range(generations):
        ...
        # Zapisywanie najlepszego wyniku w bieżącej generacji
...
for N in Ns:
    ...
    best_u, best_fitness, fitness_history = genetic_algorithm(N)
    results[N] = (best_u, best_fitness, fitness_history)
...
plt.figure(figsize=(12, 8))
for N in [5, 20, 45]:  # Wybieramy reprezentatywne wartości N
    _, _, fitness_history = results[N]
    plt.plot(range(len(fitness_history)), fitness_history, label=f"N={N}")

plt.title("Poprawa wskaźnika J w trakcie iteracji dla wybranych N")
